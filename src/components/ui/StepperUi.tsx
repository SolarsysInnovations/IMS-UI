...
const CustomStepIcon: React.FC<StepIconProps & { status?: string }> = (
  props,
) => {
...
const StageStepper: React.FC<StageStepperProps> = ({ stages }) => {
  ...
  const stageLabels = Object.keys(stages).map((key, index) =>
    stages[key as keyof StagesType] !== 'null'
      ? stages[key as keyof StagesType]
      ...
      >
        {stageLabels.map((label, index) => {
          ...
          if (isStepFailed(index)) {
            labelProps.optional = (
              <Typography
                sx={{ fontSize: '9px', whiteSpace: 'nowrap' }}
                variant="caption"
                color="error"
              >
                {' '}
          ...
          return (
            <Step
              sx={{ margin: '0 5px', width: '30px' }}
              key={index}
              active={
                stages[`stage${index + 1}` as keyof StagesType] !== 'null'
              }
            >
              <StepLabel
                StepIconComponent={(props) => (
                  <CustomStepIcon {...props} status={status} />
                )}
                {...labelProps}
              >
