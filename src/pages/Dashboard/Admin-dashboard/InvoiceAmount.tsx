...
const transformInvoiceData = (data: any) => {
  ...
  const total = data?.total ?? { totalAmount: 0, noOfInvoices: 0 };
  const paid = data?.paid ?? { totalAmount: 0, noOfInvoices: 0 };
  const unPaid = data?.unPaid ?? { totalAmount: 0, noOfInvoices: 0 };
...
const AdminDashboardInvoiceOverviewAmount = ({
